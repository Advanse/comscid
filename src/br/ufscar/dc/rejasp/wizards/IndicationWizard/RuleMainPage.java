...
//		bChanged = false;
	...
	public void onEnterPage() {
		...
		for (int i = 0; i < rules.size(); i++){
			...
			if ( rule.getTarget().equals(MatchText.STRING_LITERAL )) {
				sItem = "string literal that ";
			} else if ( rule.getTarget().equals(MatchText.VARIABLE_NAME )) {
				sItem = "variable name that ";
			} else if ( rule.getTarget().equals(MatchText.METHOD_NAME )) {
				sItem = "method name that ";
			} else if ( rule.getTarget().equals(MatchText.CLASS_NAME )) {
				sItem = "class name that ";
			}
			
			if ( rule.getRule().equals(MatchText.CONTAINS) ) {
				sItem += "contains "; 
			} else if ( rule.getRule().equals(MatchText.STARTS_WITH )) {
				sItem += "starts with "; 
			} else if ( rule.getRule().equals(MatchText.ENDS_WITH )) {
				sItem += "ends with ";
			} 
