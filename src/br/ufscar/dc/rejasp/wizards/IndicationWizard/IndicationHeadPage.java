...
public class IndicationHeadPage extends WizardPage implements Listener {
	...
	public void createControl(Composite parent) {
		...
		comboIndications = new Combo(composite, SWT.MULTI | SWT.READ_ONLY);
		comboIndications.add(NULL);
		comboIndications.add(CONNECTION);
		comboIndications.add(TRANSACTION);
		comboIndications.add(SYN);
		comboIndications.add(LOGGING);
		comboIndications.add(PERSISTENT);
		comboIndications.add(OBSERVER);
		comboIndications.add(SINGLETON);
	...
	public void handleEvent(Event event) {
	    ...
	    // Is description empty?
    		...
    		status = new Status(IStatus.ERROR, "not_used", 0, 
    				"It's required enter an indication description", null);
	...
	public void onEnterPage() {
		...
		if ( ! wizard.newIndication ) {
			...
			setDescription("Enter a indication name and description");
		}
	...
	public IWizardPage getNextPage() {
		...
		indication.setIndicationType(comboIndications.getText());
		//properties.setProperty(txtName.getText(), comboIndications.getText());
		if(wizard.newIndication)
