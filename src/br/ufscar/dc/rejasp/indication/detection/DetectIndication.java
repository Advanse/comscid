...
import br.ufscar.dc.dmasp.controller.Dmasp;
...
/**
 * @author Daniel Kawakami This is class is responsible for detect indications
 *         in the source code of only one file and show them in the indication
 *         tree view and problem view.
 */
public class DetectIndication {

	// Paulo
	private static Dmasp dmasp;
	...
	private IFile file;
	/**
	 * List of imports with indication found in source code. If the indication
	 * import is on-demand. The list will have only one element.
	 */
	...
	/**
	 * List of local variables. This variables have visibility only inside the
	 * block that they are declared.
	 */
	...
	/**
	 * Reference to the last typeInfo created, which has another typeInfo
	 * currently being analised. If no typeInfo is being analised or the current
	 * typeInfo isn't a enclosured typeInfo, this field is null.
	 */
	private TypeInfo lastTypeInfo;

	/**
	 * Reference to the typeInfo currently being used. It is used by
	 * TypeDeclarationStatement
	 */
	private TypeInfo currentTypeInfo;

	/**
	 * It stores all types found. It's used to find out all indication types
	 * found in an method.
	 */
	 ...
	 * @param file
	 */
	public DetectIndication(IndicationTree.ViewContentProvider provider,
			TreeObject fileNode, CompilationUnit unit, IFile file) {
		...
		lastIndication = null;
		dmasp = DmaspFactory.getInstance();
	}

	/**
	 * @return Returns important informations about this file that wil be used
	 *         to refactor field and methods
	 */
	...
	public boolean detectIndication() {
		...
		if (indication != null && indication.getPackages().size() > 0)
			...
			for (int i = 0; i < lstImports.size(); i++) {
				...
				for (int j = 0; j < indication.getPackages().size(); j++) {
					libraryIndication = (IndicationPackage) indication
							.getPackages().get(j);
					String sIndicationType = impDeclaration.getName()
							.getFullyQualifiedName();
					sIndicationType = sIndicationType.substring(sIndicationType
							.lastIndexOf('.') + 1, sIndicationType.length());
					String sPackageOfImport = impDeclaration.getName()
							.getFullyQualifiedName();
					if (!impDeclaration.isOnDemand()) {
						if (sPackageOfImport.lastIndexOf('.') == -1) // Tipo
																		// importado
																		// não
																		// tem
																		// pacote
							...
							sPackageOfImport = sPackageOfImport.substring(0,
									sPackageOfImport.lastIndexOf('.'));
					}
					if (sPackageOfImport.equals(libraryIndication.getName()))
						if (impDeclaration.isOnDemand()) {
							...
							createIndication(impDeclaration);
							if (lstIndicationImports.size() == 0
									|| !((String) lstIndicationImports.get(0))
											.equalsIgnoreCase("*")) {
								lstIndicationImports.add(sPackageOfImport + "."
										+ sIndicationType);
							}
		...
		// No indication import was found and preferences accept only indication
		// based on imported types,
		// let's only find enclosured types, methods and fields and store them
		// in file info
		/*if (ifFiltroByType && lstIndicationImports.isEmpty()) {
			...
			for (int i = 0; i < lstTypeDeclarations.size(); i++)
				if (lstTypeDeclarations.get(i) instanceof TypeDeclaration) {
					fileInfo
							.addType(getTypeInfo((TypeDeclaration) lstTypeDeclarations
									.get(i)));
				}
			//return false;
		}*/
	...
	/**
	 * This method is used only when no indication was found in a
	 * typeDeclaration. All information about a type is collected.
	 * 
	 * @param type
	...
	private TypeInfo getTypeInfo(TypeDeclaration type) {
		...
		for (int i = 0; i < type.modifiers().size(); i++)
			if (type.modifiers().get(i) instanceof Modifier)
				sModifiers += ((Modifier) type.modifiers().get(i)).getKeyword()
						.toString()
						+ " ";
		if (sModifiers.length() > 0)
			sModifiers = sModifiers.substring(0, sModifiers.length() - 1);

		TypeInfo typeInfo = new TypeInfo(type.getName().getIdentifier(),
				sModifiers, type.isInterface());
		...
		for (int i = 0; i < fields.length; i++) {
			...
			for (int j = 0; j < fields[i].modifiers().size(); j++)
				if (fields[i].modifiers().get(j) instanceof Modifier)
					sModifiers += ((Modifier) fields[i].modifiers().get(j))
							.getKeyword().toString()
							+ " ";
			if (sModifiers.length() > 0)
				sModifiers = sModifiers.substring(0, sModifiers.length() - 1);
			for (int j = 0; j < fields[i].fragments().size(); j++) {
				fragment = (VariableDeclarationFragment) fields[i].fragments()
						.get(j);
				...
				fieldInfo = new FieldInfo(fragment.getName().getIdentifier(),
						fields[i].getType().toString(), sModifiers,
						sInitializer);
		...
		for (int i = 0; i < methods.length; i++) {
			...
			for (int j = 0; j < methods[i].modifiers().size(); j++)
				if (methods[i].modifiers().get(j) instanceof Modifier)
					sModifiers += ((Modifier) methods[i].modifiers().get(j))
							.getKeyword().toString()
							+ " ";
			if (sModifiers.length() > 0)
				sModifiers = sModifiers.substring(0, sModifiers.length() - 1);
	...
	private boolean detectIndication(TypeDeclaration typeDeclaration) {
		 ...
		 * < TypeParameter { , TypeParameter } > ] [ extends Type ] [ implements
		 * Type { , Type } ] { { ClassBodyDeclaration | ; } }
		...
		for (int i = 0; i < typeDeclaration.modifiers().size(); i++)
			if (typeDeclaration.modifiers().get(i) instanceof Modifier)
				sModifiers += ((Modifier) typeDeclaration.modifiers().get(i))
						.getKeyword().toString()
						+ " ";
		if (sModifiers.length() > 0)
			sModifiers = sModifiers.substring(0, sModifiers.length() - 1);

		TypeInfo typeInfo = new TypeInfo(typeDeclaration.getName()
				.getIdentifier(), sModifiers, typeDeclaration.isInterface());
		...
		if (typeDeclaration.getSuperclassType() != null) {
			// Paulo 22/11 - Heranca 
			Type t = typeDeclaration.getSuperclassType();
			ITypeBinding tb = t.resolveBinding();
			if (tb != null) {
				dmasp.createPendencyInheritance(unit.getPackage(), typeDeclaration, tb);				
			}
		...
		if (typeDeclaration.superInterfaceTypes() != null
				&& typeDeclaration.superInterfaceTypes().size() > 0) {
			typeInfo.setSubType(true);
			List<Type> interfaceTypes = typeDeclaration.superInterfaceTypes();
			for (int i = 0; i < interfaceTypes.size(); i++) {

				// Paulo 22/11 - Implementação de Interfaces				
				Type tDecl = interfaceTypes.get(i);
				ITypeBinding tb = tDecl.resolveBinding();
				if (tb != null) {
					dmasp.createPendencyImplements(unit.getPackage(), typeDeclaration, tb);				
				}
				// Paulo 22/11 - Implementação de Interfaces
		...
		boolean isIndicationName = isIndicationClassName(typeDeclaration.getName());
		boolean isIndicationType = isIndicationType(typeDeclaration.getName().getFullyQualifiedName());		
		if ((isIndicationName) || (isIndicationType)) {
			createIndication(typeDeclaration);
			if (isIndicationName) {
				dmasp.createPendencyClass(unit.getPackage(), typeDeclaration, getActiveIndication(), Dmasp.PRIMARYCC, true);
			}
			else {
				dmasp.createPendencyClass(unit.getPackage(), typeDeclaration, getActiveIndication(), Dmasp.PRIMARYCC, false);
			}
		}
		...
		for (int j = 0; j < fields.length; j++) {
			...
			if (isIndicationType(fields[j].getType())) {
				...
				for (int k = 0; k < fieldFragments.size(); k++) {
					...
					fieldFragment = (VariableDeclarationFragment) fieldFragments
							.get(k);
					if (fieldFragment.getInitializer() == null)
						sInitializer = "";
					else
						sInitializer = " = "
								+ fieldFragment.getInitializer().toString();

					lstFields.addElement(new VariableInfo(fields[j].getType()
							.toString(), fieldFragment.getName()
							.getIdentifier()));

					// Store field info
					FieldInfo fieldInfo = new FieldInfo(fieldFragment.getName()
							.getIdentifier(), fields[j].getType().toString(),
							sModifiers, sInitializer);
			...
			// Let's search indications in field fragments
				...
				for (int k = 0; k < fieldFragments.size(); k++) {
					...
					if (isIndicationVariableDeclarationFragment((VariableDeclarationFragment) fieldFragments
							.get(k))) {
						if (fieldFragments.size() == 1) {
							...
							createIndication((VariableDeclarationFragment) fieldFragments
									.get(k));
						...
						lstFields.addElement(new VariableInfo(fields[j]
								.getType().toString(),
								((VariableDeclarationFragment) fieldFragments
										.get(k)).getName().getIdentifier()));
						fieldInfo.setIndication(true);
						typeInfo.setIndication(true);
						lastIndication.addRefatoring(TreeObject.REFACTOR_FIELD);
						lastIndication.addInfo(fieldInfo);
			...
			if (hasIndicationField) {
				
				// Paulo 16/11/10
				int indicationType;
				if (isIndicationType(typeDeclaration.getName().getFullyQualifiedName())) indicationType = Dmasp.PRIMARYCC;
				else indicationType = Dmasp.SECONDARYCC;				
				// Paulo 16/11/10
					
				dmasp.createPendencyAttribute(unit.getPackage(), typeDeclaration, fields[j], getActiveIndication(), Dmasp.PRIMARYCC);
				dmasp.createPendencyClass(unit.getPackage(), typeDeclaration, getActiveIndication(), indicationType, false);
				// Paulo
		...
		for (int j = 0; j < methods.length; j++) {
			...
			pckDecl = unit.getPackage();
			typeDecl = typeDeclaration;
			methodDecl = methods[j];			
			...
			for (int k = 0; k < methods[j].modifiers().size(); k++)
				if (methods[j].modifiers().get(k) instanceof Modifier)
					sModifiers += ((Modifier) methods[j].modifiers().get(k))
							.getKeyword().toString()
							+ " ";
			if (sModifiers.length() > 0)
				sModifiers = sModifiers.substring(0, sModifiers.length() - 1);
			...
			MethodInfo methodInfo = new MethodInfo(methods[j].getName()
					.getIdentifier(), sType, sModifiers);
			...
			if (isIndicationMethodName(methods[j].getName())) {
				System.out.println("fazzzzzzzzzzzzzz");
				createIndication(methods[j]);
				lastIndication.addRefatoring(TreeObject.REFACTOR_METHOD);
				lastIndication.addInfo(methodInfo);
				bMethodHasIndication = true;
			}
			...
			if (methods[j].getBody() != null) {
				...
				for (int k = 0; k < methods[j].parameters().size(); k++) {
					argument = (SingleVariableDeclaration) methods[j]
							.parameters().get(k);
					/*
					 * SingleVariableDeclaration: { ExtendedModifier } Type [
					 * ... ] Identifier { [] } [ = Expression ]
					 */
					if (isIndicationType(argument.getType())) {
						lstMethodArguments.add(argument.getName()
								.getIdentifier());
						createIndication(argument);
						lastIndication
								.addRefatoring(TreeObject.REFACTOR_METHOD);
						lastIndication.addInfo(methodInfo);
						bMethodHasIndication = true;
				...
				// References to TreeObjects related to statements in method
				// that don't
				...
				// Storage boolean values that indicate if the statements have
				// indication or not
				...
				try {
					...
					for (int k = 0; k < statements.size(); k++){
						...
						//Heitor
						if (statements.get(k) instanceof VariableDeclarationStatement){
							VariableDeclarationStatement variableDeclarationStatement = (VariableDeclarationStatement) statements.get(k);							
							List fragments = variableDeclarationStatement.fragments();
							if (isIndicationType(variableDeclarationStatement.getType())) {
								for (int i = 0; i < fragments.size(); i++){
									dmasp.createPendencyVariable(unit.getPackage(), typeDeclaration, methods[j], variableDeclarationStatement, ((VariableDeclarationFragment) fragments.get(i)), getActiveIndication());
								}
							}
						}
				...
				for (int k = 0; k < lstTreeObjects.size(); k++) {
					...
					treeObject
							.addRefatoring(methodStInfo
									.getRefactoring((Statement) treeObject
											.getAstNode()));
				}

				// Stores informations about method
				if (bMethodHasIndication) {
					// Extract duplicated types in list of types found in this
					// method
					int k = 0, l;
					...
					if (isIndicationMethodName(methods[j].getName())) indicationType = Dmasp.PRIMARYCC;
					else indicationType = Dmasp.SECONDARYCC;
					dmasp.createPendencyOperation(unit.getPackage(), typeDeclaration, methods[j], getActiveIndication(), indicationType);
					...
					while (k < lstTypesFoundInMethod.size()) {
						...
						while (l < lstTypesFoundInMethod.size()) {
							if (lstTypesFoundInMethod.get(k).equals(
									lstTypesFoundInMethod.get(l)))
		...
		if (typeDeclaration.getTypes() != null
				&& typeDeclaration.getTypes().length > 0) {
			TypeDeclaration[] arrayTypes = typeDeclaration.getTypes();
			// For each type declaration
			for (int i = 0; i < arrayTypes.length; i++) {
				// The enclosured types about to be analised need to know what
				// type
		...
		// In the end of this class, we need to free the level (context) of
		// fields created by this class
	...
	private boolean detectIndication(EnumDeclaration enumDeclaration) {
		...
		/*
		 * if ( types != null ) for ( int i = 0; i <
		 * lstIndicationImports.size(); i++ ) if ( isIndicationType(
		 * (Type)lstIndicationImports.get(i)) ) createIndication(
		 * (Type)lstIndicationImports.get(i) );
		 */
		if (types != null)
		...
		if (constants != null)
			for (int i = 0; i < constants.size(); i++) {
				 ...
				 */
				EnumConstantDeclaration constantDeclaration = (EnumConstantDeclaration) constants
						.get(i);
	...
	/**
	 * Search for a indication in a statement. If the statements contains other
	 * statement(s), each statement is analysed calling this method recursively.
	 * 
	 * @param statement
	 *            : statement to be analysed
	 * @return Returns true if indication was found
	 */
	private boolean detectIndicationStatement(Statement statement,
			MethodInfo methodInfo) {
		 ...
		 */
			...
			if (breakStatement.getLabel() != null
					&& isIndicationIdentifier(breakStatement.getLabel()
							.getIdentifier())) {
				createIndication(statement);
				bIndicationFound = true;
			}
		 ...
		 */
			...
			SingleVariableDeclaration formalParameter = enhancedForStatement
					.getParameter();
			// Is the formal parameter a indication?
			if (isIndicationType(formalParameter.getType())) {
				createIndication(formalParameter);
				bIndicationFound = true;
			} else if (isIndicationIdentifier(formalParameter.getName()
					.getIdentifier())) {
				createIndication(formalParameter);
				bIndicationFound = true;
			}
		 ...
		 * Statement ForInit: Expression { , Expression } ForUpdate: Expression
		 * { , Expression }
		 */
		 ...
		 */
			...
			if (isIndicationIdentifier(labeledStatement.getLabel()
					.getIdentifier())) {
		 ...
		 */
			...
			if (!bIndication
					&& isIndicationExpression(superConstructorInvocation
							.getExpression()))
				bIndication = true;
			if (!bIndication) {
		 ...
		 */
			...
			for (int i = 0; i < catchs.size(); i++)
				if (isIndicationType(((CatchClause) catchs.get(i))
						.getException().getType())) {
					createIndication((CatchClause) catchs.get(i));
					bIndicationFound = true;
				} else if (isIndicationIdentifier((((CatchClause) catchs.get(i))
						.getException().getName().getIdentifier()))) {
					createIndication((CatchClause) catchs.get(i));
					bIndicationFound = true;
				} else if (isIndicationExpression((((CatchClause) catchs.get(i))
						.getException().getInitializer()))) {
					createIndication((CatchClause) catchs.get(i));
					bIndicationFound = true;
				}
		}
		 ...
		 */
			...
			if (typeDeclarationStatement.getDeclaration() instanceof TypeDeclaration) {
				...
				detectIndication((TypeDeclaration) typeDeclarationStatement
						.getDeclaration());
			} else if (typeDeclarationStatement.getDeclaration() instanceof EnumDeclaration)
				detectIndication((EnumDeclaration) typeDeclarationStatement
						.getDeclaration());
		}
		/*
		 ...
		 */
			...
			if (isIndicationType(variableDeclarationStatement.getType())) {
				...
				bIndicationFound = true;
				for (int i = 0; i < fragments.size(); i++)
					variables.addElement(new VariableInfo(
							variableDeclarationStatement.getType().toString(),
							((VariableDeclarationFragment) fragments.get(i))
									.getName().getIdentifier()));
			} else {
				for (int i = 0; i < fragments.size(); i++)
					if (isIndicationVariableDeclarationFragment((VariableDeclarationFragment) fragments
							.get(i))) {
						if (fragments.size() == 1)
							createIndication(statement);
						else
							createIndication((VariableDeclarationFragment) fragments
									.get(i));
						variables
								.addElement(new VariableInfo(
		...
		if (statements != null) {
			...
			for (int i = 0; i < statements.size(); i++) {
				if (bIndicationFound)
					detectIndicationStatement((Statement) statements.get(i),
							methodInfo);
				else
					bIndicationFound = detectIndicationStatement(
							(Statement) statements.get(i), methodInfo);
			}
			if (bNewBlock)
				variables.removeLevel();
		}
	...
	// Paulo
	private boolean isIndicationType(String sType) {
		Indication indication = getActiveIndication();
		if (sType.indexOf('.') == -1) {
			...
			for (int i = 0; i < indication.getPackages().size(); i++) {
				...
				/* Paulo. Do jeito que foi implementado um tipo Date de um pacote
				 * qualquer é capturado por causa de um tipo Date do pacote java.sql.
				*/
				
				if (((IndicationPackage) indication.getPackages()
						.get(i)).isType(sType)) {
					lstTypesFoundInMethod
							.add(((IndicationPackage) indication
									.getPackages().get(i)).getName()
									+ "." + sType);
					return true;
				}
			}
			...
			for (int i = 0; i < lstIndicationImports.size(); i++) {
				// Verify if the type was imported and belongs to
				// current indication
				String sImportedType = ((String) lstIndicationImports
						.get(i));
				sImportedType = sImportedType.substring(sImportedType
						.lastIndexOf('.') + 1, sImportedType.length());
				if (sType.compareTo(sImportedType) == 0) {
					lstTypesFoundInMethod
							.add((String) lstIndicationImports.get(i));
					return true;
				}
			}
		}
		// Paulo 27/04 - Não procurava em pacotes sem imports		
		/*}
		} else {
	...
	/**
	 * The methods detectIndicationStatement is called recursively in order to
	 * find indications in statements. The method isIndicationExpression also
	 * calls itself recursively in order to find indications in expressions. One
	 * of finish condition is when a indication type is found. This method
	 * verify if the type is an indication type.
	 * 
	 * @param type
	...
	private boolean isIndicationType(Type type) {
		...
		return isIndicationType(sType);

	}

	/**
	 * This method looks for indications in an expression. Some expressions have
	 * other expressions, and this expressions have other expressions, and so
	 * on. So this method is called ina recursuvely way. *
	 * 
	 * @param expression
	...
	private boolean isIndicationExpression(Expression expression) {
		 ...
		 * [ ] } new TypeName [ < Type { , Type } > ] [ Expression ] { [
		 * Expression ] } { [ ] } new PrimitiveType [ ] { [ ] } ArrayInitializer
		 ...
		 */
			...
			for (int i = 0; i < arrayCreation.dimensions().size(); i++)
				if (isIndicationExpression((Expression) arrayCreation
						.dimensions().get(i)))
					return true;
		 ...
		 */
			...
			for (int i = 0; i < arrayInitializer.expressions().size(); i++)
				if (isIndicationExpression((Expression) arrayInitializer
						.expressions().get(i)))
					return true;
		}
		/*
		...
		/*
		 * ClassInstanceCreation: [ Expression . ] new [ < Type { , Type } > ]
		 * Type ( [ Expression { , Expression } ] ) [ AnonymousClassDeclaration
		 * ]
		 */
		 ...
		 */
			...
			if (isIndicationExpression(conditionalExpression.getExpression()))
				return true;
			else if (isIndicationExpression(conditionalExpression
					.getThenExpression()))
				return true;
			else if (isIndicationExpression(conditionalExpression
					.getElseExpression()))
		 ...
		 */
			...
			for (int i = 0; i < infixExpression.extendedOperands().size(); i++)
				if (isIndicationExpression((Expression) infixExpression
						.extendedOperands().get(i)))
					return true;
		}
		/*
		 ...
		 */
			...
			if (isIndicationType(instanceofExpression.getRightOperand()))
				return true;
			else if (isIndicationExpression(instanceofExpression
					.getLeftOperand()))
				return true;
		}
		/*
		 ...
		 */
			...
			System.out.println("xxxxxxxxxxx " + methodInvocation.getName());

			/*if (isIndicationMethodName(methodInvocation.getName())) {
				int indicationType;
				dmasp.createPendencyOperation(unit.getPackage(), typeDecl, methodDecl, getActiveIndication(), Dmasp.SECONDARYCC);
				if (isIndicationType(typeDecl.getName().getFullyQualifiedName())) indicationType = Dmasp.PRIMARYCC;
				else indicationType = Dmasp.SECONDARYCC;				
				dmasp.createPendencyClass(unit.getPackage(), typeDecl, getActiveIndication(), indicationType, false);
			}*/
			dmasp.createPendencyCalledOperation(pckDecl, typeDecl, methodDecl, methodInvocation.getName().getFullyQualifiedName());
			
			// Paulo			
			IMethodBinding methodBinding = methodInvocation.resolveMethodBinding();
			if (methodBinding != null) {
				ITypeBinding typeBinding = methodBinding.getDeclaringClass();
				if ((typeBinding != null) && (pckDecl !=null) && (typeDecl != null) && (methodDecl != null)) {
					System.out.println("novo*novo*novo*novo" + typeBinding.getQualifiedName() + "id method decl: " + methodDecl.getName() + " - id method called: " + methodBinding.getName());
					
					// 22/12
					dmasp.createPendencyCalledOperation(pckDecl, typeDecl, methodDecl, typeBinding, methodBinding);
					// 22/12
					
					// Paulo 07/09 - Não capturava metodos estaticos 
					if (isIndicationType(typeBinding.getName())) return true;
					// Paulo 07/09 - Não capturava metodos estaticos
				}
			}			
			/*IType is = (IType) typeBinding.getJavaElement();
			try {
				org.eclipse.jdt.core.ITypeHierarchy hi = is.newTypeHierarchy(null);
				
				IType[] types = hi.getAllSubtypes(is);;
				for(int i = 0; i<types.length; i++) {
					System.out.println("sssssssss: " + types[i].getElementName());
				}
			...
			if (binding == null) {System.out.println("errrrrrrrrrrr"); } 
			else {
				
				ITypeBinding[] itv = binding.getParameterTypes();
				System.out.println("mthshreoehsoh: " + binding.getDeclaringClass().getQualifiedName() + " - " + binding.isGenericMethod());
				for (int i = 0; i < itv.length; i++) {
					System.out.println("param: " + itv[i].getName());					
				}
			}*/
			//Paulo			
			for (int i = 0; i < methodInvocation.typeArguments().size(); i++)
				if (isIndicationType((Type) methodInvocation.typeArguments()
						.get(i)))
					return true;
			if (methodInvocation.getExpression() != null
					&& isIndicationExpression(methodInvocation.getExpression()))
				return true;
			if (isIndicationIdentifier(methodInvocation.getName()
					.getIdentifier()))
				return true;
			if (methodInvocation.arguments() != null)
				for (int i = 0; i < methodInvocation.arguments().size(); i++)
					if (isIndicationExpression((Expression) methodInvocation
							.arguments().get(i)))
						return true;
		}
		/*
		 ...
		 */
			...
			if (isIndicationExpression(superFieldAccess.getQualifier()))
				return true;
			if (isIndicationIdentifier(superFieldAccess.getName()
					.getIdentifier()))
				return true;
		}
		/*
		 ...
		 */
			...
			if (superMethodInvocation.typeArguments() != null
				...
				for (int i = 0; i < superMethodInvocation.typeArguments()
						.size(); i++)
					if (isIndicationType((Type) superMethodInvocation
							.typeArguments().get(i)))
						return true;
			if (isIndicationIdentifier(superMethodInvocation.getName()
					.getIdentifier()))
				return true;
			...
			if (superMethodInvocation.arguments() != null)
				for (int i = 0; i < superMethodInvocation.arguments().size(); i++)
					if (isIndicationExpression((Expression) superMethodInvocation
							.arguments().get(i)))
						return true;
		}
		/*
		 ...
		 */
			...
			// For each fragment
			for (int i = 0; i < variableDeclarationExpression.fragments()
					.size(); i++) {
				/*
				 * VariableDeclarationFragment: Identifier { [] } [ = Expression
				 * ]
				 */
				fragment = (VariableDeclarationFragment) variableDeclarationExpression
						.fragments().get(i);
				if (!bIndicationType) {
					if (isIndicationExpression(fragment.getInitializer()))
						variables.addElement(new VariableInfo(
								variableDeclarationExpression.getType()
										.toString(), fragment.getName()
										.getIdentifier()));
				} else
					variables.addElement(new VariableInfo(
							variableDeclarationExpression.getType().toString(),
	...
	/**
	 * This method verify if the variable has a name that could be an
	 * indication.
	 * 
	 * @param name
	 *            variable name
	...
	/**
	 * This method verify if the method has a name that could be an
	 * indication.
	 * 
	 * @param name
	 *            method name
	 * @return true if the method has an indication name or false, otherwise.
	 */
	private boolean isIndicationMethodName(SimpleName name) {
		String sName = name.getIdentifier();
		return isIndicationMatch(sName, MatchText.METHOD_NAME);
	}

	/**
	 * This method verify if the class has a name that could be an
	 * indication.
	 * 
	 * @param name
	 *            class name
	 * @return true if the class has an indication name or false, otherwise.
	 */
	private boolean isIndicationClassName(SimpleName name) {
		String sName = name.getIdentifier();
		return isIndicationMatch(sName, MatchText.CLASS_NAME);
	}
	
	/**
	 * Create an indication node in the model of indication tree and include the
	 * indication in the list of problems in problem view.
	 * 
	 * @param node
	 *            it's the statement with indications.
	 * @return
	 */
	private boolean createIndication(ASTNode node) {
		...
		try {
			...
			marker.setAttribute(IMarker.CHAR_END, node.getStartPosition()
					+ node.getLength());
			marker.setAttribute(IMarker.LINE_NUMBER, unit.getLineNumber(node
					.getStartPosition()));
	 ...
	 * find indications in statements. The method isIndicationExpression also
	 * calls itself recursively in order to find indications in expressions. One
	 * of finish condition is when a variable of indication type is found. This
	 * method verify if the variable has an indication type.
	 * 
	...
	private boolean isIndicationIdentifier(String sQualifiedName) {
		...
		if (sParts.length > 1) {
			int i;
			// Compara com os elementos da lista de atributos da classe /
			// interface
			fields = lstFields.getAllElements();
			for (i = 0; i < fields.size(); i++) {
				if (((VariableInfo) fields.get(i)).getName().compareTo(
						sParts[sParts.length - 2]) == 0) {
					return true;
				}
			}
			// Compara com os elementos da lista de argumentos do método
			for (i = 0; i < lstMethodArguments.size(); i++) {
				if (((String) lstMethodArguments.get(i))
						.compareTo(sParts[sParts.length - 2]) == 0) {
					return true;
				}
			}
			...
			for (i = 0; i < varLst.size(); i++) {
				if (((VariableInfo) varLst.get(i)).getName().compareTo(
						sParts[sParts.length - 2]) == 0) {
					return true;
				}
			}
	 ...
	 */
	private boolean isIndicationVariableDeclarationFragment(
			VariableDeclarationFragment fragment) {
		/*
		 * VariableDeclarationFragment: Identifier { [] } [ = Expression ]
		 */
	 ...
	 * 
	 * @return: return the first active indication or null if there is no active
	 *          indication.
	 */
	 ...
	 * 
	 * @param sContent
	 *            text where the indication will be search.
	 * @param sTarget
	 *            string literal or variable name
	...
	private boolean isIndicationMatch(String sContent, String sTarget) {
		...
		System.out.println("--------------Variavel: " + sContent);
		
		for (int i = 0; i < stringMatches.size(); i++) {

			System.out.println("**************Regra: " + ((MatchText) stringMatches.get(i)).getRule() + " - " + ((MatchText) stringMatches.get(i)).getTarget());			
			
			if (((MatchText) stringMatches.get(i)).getTarget().equalsIgnoreCase(sTarget)) {				
				...
				if (match.isCaseSensity()) {
					if (match.getRule().equalsIgnoreCase(MatchText.CONTAINS)) {
						...
						//return false;
					} else if (match.getRule().equalsIgnoreCase(
							MatchText.STARTS_WITH)) {
						for (int j = 0; j < lstWords.size(); j++) {
					...
					if (match.getRule().equalsIgnoreCase(MatchText.CONTAINS)) {
						for (int j = 0; j < lstWords.size(); j++) {
							System.out.println("Entrou no contains. Nome: " + sContent + " - Tipo: " + sTarget + " - Palavra: " + lstWords.get(j));

							if (sContent.toLowerCase().contains(
									((String) lstWords.get(j)).toLowerCase())) {
								return true;
							}
						}
						//return false;
							...
							MatchText.STARTS_WITH)) {
						for (int j = 0; j < lstWords.size(); j++) {
							System.out.println("Entrou no starts with. Nome: " + sContent + " - Tipo: " + sTarget + " - Palavra: " + lstWords.get(j));
							
							if (sContent.toLowerCase().startsWith(
									((String) lstWords.get(j)).toLowerCase())) {
								return true;
							}
						}
						//return false;
							...
							MatchText.ENDS_WITH)) {
						for (int j = 0; j < lstWords.size(); j++) {
							System.out.println("Entrou no ends with. Nome: " + sContent + " - Tipo: " + sTarget + " - Palavra: " + lstWords.get(j));
							
							if (sContent.toLowerCase().endsWith(
									((String) lstWords.get(j)).toLowerCase())) {
								return true;
							}
						}
	 ...
	 * 
	 * @param statement
	 * @return true if the statement always has an associated statement or
	 *         false, otherwise.
	 */
	...
	/**
	 * Get the type name of a type object. A type object has six varitions. Each
